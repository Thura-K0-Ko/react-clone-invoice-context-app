...
const Record = ({ id, name, price, quantity, cost, index }) => {
  ...
  return (
    <tr className="group odd:bg-indigo-300 even:bg-cyan-400">
      ...
      <th
        scope="row"
        className=" py-3 px-5 text-start  text-gray-900 font-medium"
      >
        {name}
      </th>
      ...
      <td className=" py-3 px-5 text-end relative">
        <span>{cost.toFixed(2)}</span>
        <button
          onClick={() => removeRecord(id)}
          className=" bg-green-100 rounded absolute translate-x-3 group-hover:translate-x-0 py-1 px-1 opacity-0 group-hover:opacity-100 duration-75  "
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
